...
int main(int argc, char* argv[]) {
  try {
    ...
    if (window.isOpen()) {
      ...
      graph.write_legend("Legend", "S=blue", "I=red", "R=green", parameters);
      window.display();
    }

    // game loop for the first window
    while (window.isOpen()) {
      sf::Event event;
    ...
    if (window2.isOpen()) {
      ...
      std::string parameters = "N=" + std::to_string(pandemic.getPopulation()) +
                               "\npar1=" + std::to_string(beta) +
                               "\npar2=" + std::to_string(gamma);
      graph2.write_legend("Legend", "S=blue", "I=red", "R=green", parameters);
      window2.display();
    }

    // game loop for the second window
    ...
    /* game loop for the best rendering of the programm:
